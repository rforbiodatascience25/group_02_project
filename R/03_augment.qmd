...
analysis_data <- analysis_data |>
  mutate(
    ...
    Strains = sc_H1N1 + sc_B + sc_H3N2,
    Response_group = if_else(n_strains <= 1, "Poor responders", "Good responders")
  )
...
plot_data <- analysis_data |>
  group_by(Age_Group, n_strains) |>
  summarise(n = n(), .groups = "drop") |>
  group_by(Age_Group) |>
  mutate(rate = 100 * n / sum(n)) |>
  ungroup() |>
  mutate(n_strains = factor(n_strains, levels = 0:3))
