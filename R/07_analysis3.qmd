...
analysis_data_final <- analysis_data_long |> 
  ...
  nest() |> 
  mutate(
    ttest_results = map(data, ~ t.test(.x$log2_signal ~ .x$Response_group, var.equal = FALSE)),
...
>>>>>>> d36ae90829936ad30cc0a4127db82d79ee2abdac
...
ggplot(aes(x = log2_FC, y = -log10(p.adj))) +
  ...
  geom_text_repel(
    ...
    max.overlaps = 50,
    nudge_y = 0.1) +
  scale_y_continuous(limits = c(0, max(-log10(analysis_data_final$p.adj), na.rm = TRUE) * 1.25)) +
  labs(
...
We observe that probes ILMN_1688780, ILMN_1739792 and ILMN_1697309 are the most differentiated probes, based on the visible distance from the center and high -log10(p.adj) value. In other words, these probes showed significantly lower baseline expression in candidates that proved to be good responders. To find the best predictor we will use a boxplot.

```{r}
#| warning: false
analysis_data_long |> 
  filter(PROBE_ID == c("ILMN_1688780", "ILMN_1739792", "ILMN_1697309")) |> 
  ...
  coord_cartesian(ylim = c(min(analysis_data_long$log2_signal, na.rm = TRUE) * 1.1, 
                           max(analysis_data_long$log2_signal, na.rm = TRUE) * 1.05)) +
  labs(
    title ="Baseline Expression of possible Predictors:\nILMN_1688780, ILMN_1739792 and ILMN_1697309",
  ...
  theme_minimal() +
  theme(legend.position = "none",
        # Fix: Angle x-axis text to prevent overlap in the facets
